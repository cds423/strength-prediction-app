...
with col_input:
    ...
    with c2:
        smm_kg = st.number_input("骨骼肌肉量 (kg)", min_value=0.0, value=30.0, help="請直接輸入報告中之骨骼肌重量 (Skeletal Muscle Mass)")
...
with col_result:
    if predict_btn:
        ...
        intensity_data = {
            "訓練目標": ["Strength (90%)", "Hypertrophy (85%)", "Lower Hypertrophy (60%)", "Endurance (<60%)"],
            "深蹲 SQ (kg)": [f"{round(pred_sq*0.9, 1)}", f"{round(pred_sq*0.85, 1)}", f"{round(pred_sq*0.6, 1)}", f"<{round(pred_sq*0.6, 1)}"],
            "臥推 BP (kg)": [f"{round(pred_bp*0.9, 1)}", f"{round(pred_bp*0.85, 1)}", f"{round(pred_bp*0.6, 1)}", f"<{round(pred_bp*0.6, 1)}"],
            "硬舉 DL (kg)": [f"{round(pred_dl*0.9, 1)}", f"{round(pred_dl*0.85, 1)}", f"{round(pred_dl*0.6, 1)}", f"<{round(pred_dl*0.6, 1)}"]
        }
